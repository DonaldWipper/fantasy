...
settings = {"sql_host":"server232.hosting.reg.ru", 
            "sql_user":"u0891874_utyan",
            "sql_passwd":"3J6i4K7e",
            "sql_db":"u0891874_fantasy"
           }
...
def init_data(_competitionId = competitionId):
    ...
    teams = [team for team in db.getDictFromQueryRes("teams") if str(team["id"]) in teams_ids] 
...
@application.route("/worldcup", methods=['GET'])
def update():
    global db
    try:
        try:
            ...
            db =  FlaskApp.sql.database(settings_local['sql_host'],  settings_local['sql_user'],  settings_local['sql_passwd'], settings_local['sql_db']) 
    except:
        try:
            db =  sql.database(settings['sql_host'],  settings['sql_user'],  settings['sql_passwd'], settings['sql_db']) 
        except:
            db =  sql.database(settings_local['sql_host'],  settings_local['sql_user'],  settings_local['sql_passwd'], settings_local['sql_db']) 
    try:
