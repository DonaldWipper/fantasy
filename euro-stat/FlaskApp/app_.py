...
def getPlacesData():
    global places
    geodata = {}
    lats = [float(ifNoneFloat(p["lat"])) for p in places]
    print(lats)
    lngs = [float(ifNoneFloat(p["lng"])) for p in places]
    cps = [int(ifNoneFloat(p["capacity"])) for p in places]
    bounds = [min(lats), max(lats), min(lngs), max(lngs)]
    print(bounds)
    bounds_capacity = [min(cps), max(cps)]
    geodata["centreX"] = (bounds[0] + bounds[1]) / 2     
    geodata["centreY"] = (bounds[2] + bounds[3]) / 2   
    geodata["width"] = abs(bounds[1] - bounds[0])   
    geodata["height"] = abs(bounds[3] - bounds[2])
    print(geodata)
    return geodata 
...
def init_data(_competitionId = competitionId):
    global stages, games, teams, places, games_update, rounds,  space, stages, db, dates, games_update2, games_playoff, games_clear, tournaments, tournamentPos, geodata 
    #initSQL()
...
def render():
    global stages, teams, places, rounds,  space, games, places, dic_slice_2_games, dates, tournaments, tournamentPos, geodata  
    sliceId = 0
...
click_events = click_events, games_clear = games_clear, slice_name = slice_name, games_playoff  = games_playoff, tournaments = tournaments, tournamentPos = tournamentPos, geodata  = geodata )
