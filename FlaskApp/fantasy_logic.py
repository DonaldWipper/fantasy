...
class sportsFantasyLogic():
    def __init__(self, _team_id, _tournament_id, _season_id, _sport_api):
        ...
        self.sort_best_rules = {'is_injured':1,
                                'is_inner_games':0,
                                'avg_season':0, 
                                'avg_goals':0,
                                'avg_minutes':0, 
    ...
    def __setAdditionalsMyFantasyTeamMetrics(self):
        ...
        self.df_my_team_all = pd.merge(self.df_my_team,
                                players_df,
                                on = 'tag_id',
                                suffixes = ('', '_all'),
                                how = "left")
    ...
    def __getAllPlayersTournamentInner(self):
        ...
        if len(self.df_all_players) == 0: 
            ...
            self.df_all_players = self.df_all_players.rename(index = str, columns = {'points': 'season'})
            dic = self.apiMethods.getPlayersDict(self.season_id, self.tournament_id, month = month, list_ids = list( self.df_all_players['id']))
            self.df_all_players['tag_id'] = self.df_all_players['id'].apply(lambda x: dic[x] if x in dic else 0)  
            print('finish to create team')
            
        players = list(self.df_all_players[self.df_all_players["status"] == False]['id']) 
        ...
        self.df_all_players = pd.merge(self.df_all_players,
    ...
    def playersInTeamLimit(self, team, club_ids = None):
        teams_limit = team[['id','club_id']].groupby(['club_id']).count().to_dict()['id']
        print(teams_limit)
        if club_ids != None:
            for w in club_ids:
    ...
    def getWorst(self, team, top=None):
        ...
        res = team.sort_values(by = list(self.sort_best_rules.keys()), ascending=ascending2)
        if (top == None):
            top = len(team)
    ...
    def __printDF(self, df_players, avg_distance, search_price, error):        
        ...
        player_avg_points = '/'.join([str("%.1f" % i) for i in list(df_players['avg_season'])])
        print ('%s with prices % s, ampluas %s, total price %s, avg dist %.1f, search price %.1f, avg_points %s, status %s'  
                % (player_names, player_prices, player_ampluas, str(df_players.sum()['price']), avg_distance, search_price, player_avg_points, error))    
        return df_players   
    ...
    def getNewTeamConcat(self, team, worst_players, best_players):
        ...
        if len(team_new) > 1:
            ...
            team_new = team_new.drop(team_new[team_new['id'].isin(worst_ids)].index) #delete worst players
        team_new = pd.concat([team_new, best_players]).sort_values(by = list(self.sort_best_rules.keys()), 
                                                                   ascending = list(self.sort_best_rules.values())).reset_index(drop=True) 
    ...
    def getBest(self,
        ...
        if (positions != []):
            ...
            for amplua in positions:
                df = team[team['amplua'] == amplua].sort_values(by = list(self.sort_best_rules.keys()), ascending = list(self.sort_best_rules.values()))
        ...
        while (self.__checkIfSearchFinish(search_list) != True):
            ...
            for cur_position in  range(len(search_list)): #for each position
                ...
                if self.__NextPlayer(search_list, cur_position):
                    ...
                    avg_distance = self.__getDistinceFromAVGPlayer(df_players, sum_price * 1.0 / 3)
                    ...
                    elif not self.__checkIfTeamLimitOk(df_players, teamsLimit, max_player_one_team ):    
